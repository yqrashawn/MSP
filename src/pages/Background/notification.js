import browser from 'webextension-polyfill';
import { show } from './popup.js';

async function notificationOnClicked(notificationId) {
  if (
    notificationId.startsWith('https://') ||
    notificationId.startsWith('chrome-extension://')
  ) {
    await show({ url: notificationId });
  }
}

function subscribeToNotificationClicked() {
  // none browser env
  // if (typeof window === 'undefined') return;
  // if (typeof global !== 'undefined') return;
  if (!browser.notifications.onClicked.hasListener(notificationOnClicked)) {
    browser.notifications.onClicked.addListener(notificationOnClicked);
  }
}

export function create(id, options = {}) {
  const {
    type = 'basic',
    message = 'Click to check DETAILED TX INFO',
    title = 'SAFE 3 TX INFO',
    iconUrl = 'icon-128.png',
  } = options;
  subscribeToNotificationClicked();
  return browser.notifications.create(id ?? undefined, {
    ...options,
    type,
    message,
    title,
    iconUrl,
  });
}
